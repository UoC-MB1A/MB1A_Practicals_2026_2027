...
doc-id: "0306"
---
```{r setup_0306, include=FALSE}
library(tidyverse)
set.seed(42)
```

# Part 6 - Sampling from a user-defined probability distribution {#sec-p6_non_standard}

::: {.callout-note .partmenu #parts-0306}
...
:::{.callout-tip .objectives #objectives-0306}
...
## Sampling from a user-defined probability distribution {#sec-user_defined .fold .bonus}
...
```{r libraries_0306, message=FALSE}
library(Runuran)
```
...
![Example data point](images/keyboard_cat.png){#fig-keyboard_cat width="50%"}
...
For a function to be a valid PDF, it must be non-negative over its range and its total area must equal 1. We can check the area in R using integrate(). Here, we will use `integrate()` with 3 arguments - the PDF, `lower` - the lower bound - and `upper` - the upper bound.
...
## User-defined PDF Exercise {#sec-exercise_pdf .fold .bonus}
...
![Example data point](images/jethro1.jpeg){#fig-cataroo1 width="50%"}
...
The value of $\pi$ is available in R as pi
...
::: {.callout-exercise #ex-cataroo .bonus}
...
![Example data point](images/jethro2.jpeg){#fig-cataroo2 width="50%"}
...
![Example data point](images/jethro3.jpeg){#fig-cataroo3 width="50%"}
...
![Example data point](images/jethro4.jpeg){#fig-cataroo4 width="50%"}
:::
:::
## Review {#sec-summary_0306}

[↑ top](#)

In this part of the practical, you have seen that simulation is not limited to the named probability distributions built into R. A probability density function can be defined directly, checked to ensure that it is valid, and used to generate simulated observations.
