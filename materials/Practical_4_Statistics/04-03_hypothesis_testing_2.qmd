...
In the previous section, we introduced one-way ANOVA using an explanatory variable with two levels. However, ANOVA can easily be expanded to look at multiple groups and multiple explanatory variables.

## One-way ANOVA with more than two groups {#sec-one_way_anova .core}
...
:::{.callout-exercise #ex-boxplot_domfam .rec}

### Box plot

[**Recommended**]{.rec-text}

In your `pollinators.qmd` Quarto document, plot a box plot showing the value of `mean_insect_mass` in your `site_metadata` data frame for the regions with each value of `dominant_family`.

Remember that:
* The R function `geom_boxplot()` draws a box plot.
* The `aes` function for this type of plot requires a categorical x variable and a continuous y variable as arguments - so you would use the arguments `x = mean_insect_mass` and `y = dominant_family`. 

:::{.callout-answer collapse="true" #ex-boxplot_domfam_ans}

```{r plot_domfam}
#| fig-alt: "Box plot comparing mean insect mass at sites dominated by four insect families: Andrenidae, Apidae, Halictidae and Syrphidae. Apidae have the highest median mass and largest IQR, followed by Syrphidae, Halictidae then Andrenidae."
ggplot(
  site_metadata,
  aes(
...
::: {.callout-exercise #ex-lm_mass_domfam .core}

### Linear model with more than two groups

[**Core**]{.core-text}

In your `pollinators.qmd` Quarto file, fit a linear model using the function `lm()` and the formula above. As for the t-test, you'll need to add the argument `data = site_metadata` so R can see the data frame. Store the output in the variable `lm_mass_domfam`. View the results using the `summary()` function.
...
## The F-distribution {#sec-f_dist .fold .extension}
...
Let's use simulation to visualise this.

::: {.callout-exercise #ext-f_distribution .extension}
...
```{r f_dist_ans3}
#| fig-alt: "Density plot of 1,000 values simulated from an F-distribution with 3 and 72 degrees of freedom. The distribution is strongly right-skewed, with most values close to zero and a long right tail. The red vertical line shows the observed F-statistic, which lies far into the right tail."
...
## Multi-way ANOVA {#sec-multi_way_anova .core}
...
The default ANOVA implementation in R tests whether each explanatory variable is associated with the response in turn. So, in the case above, explanatory1 would be tested first, followed by whether explanatory2 explains additional variation after explanatory1 has been included, and so forth.
...
#| fig-alt: "Faceted box plots comparing insect mass among dominant insect families and broad habitat types. Mean insect mass differs visibly among dominant insect families, with Apidae having the highest median and Andrenidae the lowest, as above. There are five habitats - heathland, arable, grassland/herbaceous, woodland and wetland. Amongst habitats, healthland has the highest median and arable the lowest, heathland also has a much larger IQR than the other four."
...
::: {.callout-exercise #ex-anova_twoway .core}
...
## Interactions {#sec-interactions .rec}
...
### Using simulation to visualise interactions {#sec-simulating_interactions .fold .extension}

[**Extension**]{.extension-text}

Let's simulate a dataset with an interaction, to help illustrate this concept.

In this study, there are four samples of students and the response variable is the time they took to finish their lunch, in minutes. Response time is drawn from a normal distribution with a standard deviation of 3.
...
* **Null hypothesis**: There is no association between utensil choice and finishing time.
* **Alternative hypothesis**: Utensil choice is associated with finishing time.

Food:

* **Null hypothesis**: There is no association between food choice and finishing time, after taking into account the association with utensil choice.
* **Alternative hypothesis**: Food choice is associated with finishing time, after taking into account the association with utensil choice.

Interaction:

* **Null hypothesis**: The association between utensil choice and finishing time does not vary depending on food choice.
* **Alternative hypothesis**: The association between utensil choice and finishing time varies depending on food choice.

::: {.callout-exercise #ex-simulate_interaction .extension}

### Using simulation to visualise interactions

[**Extension**]{.extension-text}

In your `prac4_exercises.qmd` Quarto file:

* Generate four simulated samples, each of 10 students, with response times normally distributed around the means above and a standard deviation of 3.
* Convert each of these into a data frame with the columns `utensil`, `food` and `time`, using the following syntax, where `time_sim` is your vector of simulated response times - `tibble(time = time_sim, food = "soup", utensil = "spoon")
* Convert these to a single data frame using the function `bind_rows()`. You can pass any number of data frames as arguments to this function and it will return a single data frame combining all of the rows (assuming the data frames all have the same columns) (e.g. as `bind_rows(df1, df2, df3)`)
...
[Generate four simulated samples, each of 10 students, with response times normally distributed around these means and a standard deviation of 3.]{.blue-text}
...
::: {.callout-exercise #ex-simulate_no_interaction .bonus}
...
::: {.callout-answer collapse="true" #ex-simulate_no_interaction_ans}
...
Now, let's try this with our data. Here, our hypotheses are:

Dominant insect family:

* **Null hypothesis**: There is no association between mean insect mass and dominant insect family.
* **Alternative hypothesis**: Mean insect mass is associated with dominant insect family.

Dominant flower order:

* **Null hypothesis**: There is no association between mean insect mass and dominant flower order, once dominant insect family has been taken into account.
* **Alternative hypothesis**: Mean insect mass is associated with dominant flower order, once dominant insect family has been taken into account.

Interaction:

* **Null hypothesis**: The association between dominant insect family and mean insect mass does not vary depending on dominant flower order.
* **Alternative hypothesis**: The association between dominant insect family and mean insect mass varies depending on dominant flower order.

::: {.callout-exercise #ex-anova_interaction .rec}
...
In this part of the practical, you extended ANOVA to compare more than two groups and to include multiple explanatory variables and interactions between explanatory variables.
...
| `anova()` | Performs an ANOVA on a fitted linear model. |
| `rf()` | Generates random values from an F-distribution. |
| `pf()` | Calculates cumulative probabilities from an F-distribution. |
| `tibble()` | Creates a data frame. |
| `bind_rows()` | Combines data frames by adding their rows together. |
| `select()` | Selects particular columns from a data frame. |
| `pivot_longer()` | Converts data from wide to long format. |
| `ggplot()` | Creates a plot using a data frame and aesthetic mappings. |
| `geom_boxplot()` | Adds a box plot to a `ggplot`. |
| `geom_density()` | Adds an estimated density distribution to a `ggplot`. |
| `geom_vline()` | Adds a vertical reference line to a `ggplot`. |
