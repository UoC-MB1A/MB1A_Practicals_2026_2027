...
* Store the resulting data frame in as a variable named `ht_df`.
...
[Write the data frame to the file `output/ht_df.tsv`.]{.blue-text}
...
| `geom_boxplot()` | Adds a box plot to a `ggplot`, allowing the distributions of a continuous variable to be compared between groups. |
| `geom_smooth()`  | Adds a fitted model to a `ggplot`; `method = "lm"` fits a linear model.                                           |
| `geom_hline()`   | Adds a horizontal reference line to a `ggplot`.                                                                   |
